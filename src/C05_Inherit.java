...
public class C05_Inherit {
	/*
	 	# 클래스 상속
	 	 ...
	 	 - 물려받은 후에는 해당 클래스를 마음대로 고쳐서 사용할 수 있다 (오버라이드 override)
	 	 - 원본의 보존하면서 새롭게 수정해볼 수 있는 것이 장점이다
	 	 - 상속 받는 클래스를 자식 클래스 또는 서브 클래스라고 부른다
	 	 - 상속 해주는 클래스를 부모 클래스 또는 슈퍼 클래스라고 부른다
...
class Programmer extends Person {
	...
	Programmer(String name, int age) {
		super(name, age);// 부모 클래스의 생성자는 꼭 있어야한다 삭제하면 난리남
	...
	// Override(짓밟다) : 자식 클래스는 부모 클래스의 메서드를 마음대로 수정해서 사용할 수 있다
	void sayHi() {
...
class OrangeTree extends Tree {
	OrangeTree() {
		// 부모 클래스에 기본 생성자가 있는 경우에는 super()를 생략할 수 있다
		super(123); 
	}
}
