...
public class B11_Array {
	...
	public static void main(String[] args) {
		/*
		  	...
		  	- 베열의 인덱스(방 번호)는 0부터 전체 길이 - 1까지 있다
		  	- 배열은 한번 크기가 정해지면 크기를 변경할 수 없다
		  	- 변수와는 다르게 배열은 생성과 동시에 모든 값이 초기화 되어 있다
		  		(정수: 0, 실수: 0.0, boolean: false, 참조형: null)// 컴퓨터의 0은 거짓 1은 참이다 
		...
		System.out.println("네 번째 스캐너: " + scanners[4]);
		
		int[] randomNumbers = new int[100];

		randomNumbers[5] = 17;
		
		System.out.println(randomNumbers[0]); // 시작은 0번 부터
		...
		// 배열.length 를 통해 배열의 전체 길이를 알 수 있다
		System.out.println("randomNumber의 길이: " + randomNumbers.length);
		System.out.println("randomNumber의 마지막 인덱스 번호: " 
				+ (randomNumbers.length - 1));
		System.out.println("scanners의 길이: " + scanners.length);
		System.out.println("scanners의 마지막 인덱스 번호: " 
				+ (scanners.length - 1));
