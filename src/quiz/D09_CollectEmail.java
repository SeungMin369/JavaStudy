...
public class D09_CollectEmail {
	...
	public static void main(String[] args) {
		...
		Pattern pattern1 = Pattern.compile("\\w+@\\w+.\\w+(\\.\\w+)?(\\.\\w+)?");
				
		Matcher matcher = pattern1.matcher(email);
		
		TreeSet<String> email_list = new TreeSet<>();
		ArrayList<String> email_list2 = new ArrayList<>();
		ArrayList<String> overlap = new ArrayList<>();

		while (matcher.find()) {
			System.out.println("찾은 것: " + matcher.group());
			System.out.println("위치: " + matcher.start() + "부터 " + (matcher.end() - 1) + "까지");
			if (email_list.contains(matcher.group())) {
				overlap.add(matcher.group());
			}
			email_list.add(matcher.group());
			email_list2.add(matcher.group());	
		}
		System.out.println(email_list.size() + "개");
